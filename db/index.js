...
async function getPostById(postId) {
  try {
    const {
      rows: [post],
    } = await client.query(`SELECT * FROM posts WHERE id=$1;`, [postId]);
    ...
    const { rows: tags } = await client.query(
      `SELECT tags.* FROM tags JOIN post_tags ON tags.id=post_tags."tagId" WHERE post_tags."postId"=$1;`,
      [postId]
    );
    ...
    const {
      ...
      `SELECT id, username, name, location FROM users WHERE id=$1;`,
      [post.authorId]
    );
...
async function getAllTags() {
  const { rows } = await client.query(`SELECT id, name FROM tags;`);
